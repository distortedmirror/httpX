...
docker create -t -p 127.0.0.1:5901:5901/tcp -p 127.0.0.1:8080:8080/tcp -p 127.0.0.1:6080:6080/tcp -p 127.0.0.1:2222:22/tcp --privileged   markbrito1975/kali:version3 bash -c 'for i in `find /etc/init.d/`;do $i restart;done; export DISPLAY=:0; cd; rm -rf httpX;git clone https://github.com/distortedmirror/httpX.git; cd httpX; Xvfb :0 -listen tcp & sleep 2; xterm -rv & mate-session &  bash --norc --noprofile' > containerid.txt ; echo Container created id in containerid.txt;cat containerid.txt 
