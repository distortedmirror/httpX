...
docker create -t -p 127.0.0.1:5901:5901/tcp -p 127.0.0.1:2222:22/tcp --privileged   kalilinux/kali bash -c 'apt-get update; apt-get upgrade;yes |apt-get install openjdk-11-jdk; yes |apt-get install xterm;yes|apt-get install git;yes|apt-get install mwm; yes|apt-get install x11-xserver-utils;yes|apt-get install net-tools;yes|apt-get install sudo;yes|apt-get install tightvncserver;yes|apt-get install vim;yes|apt-get install emacs;yes|apt-get install openssh-server; yes "" | adduser mark; printf "x\nx\n\n\n\n\n\n\n\n\n\n\n"|passwd mark; /etc/init.d/ssh start; export DISPLAY=:0; cd; git clone https://github.com/distortedmirror/httpX.git; cd httpX; ./STARTHTTPX.sh' > containerid.txt ; echo Container created id in containerid.txt;cat containerid.txt 
